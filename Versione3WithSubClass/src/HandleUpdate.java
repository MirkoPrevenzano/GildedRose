public  class HandleUpdate  {
    ...
    protected boolean isAgedBrie(Item item) { return isNotMaxQuality(item) && item.sellIn < 0; }
    protected boolean isLessTenDays(Item item) { return isNotMaxQuality(item) &&item.sellIn<11; }
    protected boolean isLessSixDays(Item item) { return isNotMaxQuality(item) &&item.sellIn<6; }
}
